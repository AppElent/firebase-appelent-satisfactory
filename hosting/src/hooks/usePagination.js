import {
  useCallback, useEffect, useMemo, useRef, useReducer
} from 'react';
...
export const limitPageBounds = (totalItems, pageSize) => (page) => Math.min(Math.max(page, 0), getTotalPages(totalItems, pageSize) - 1);
...
const getCurrentPageReducer = (rootState) => function currentPageReducer(
  state,
  action
) {
  switch (action.type) {
